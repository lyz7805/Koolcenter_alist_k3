...
start_process() {
  ...
  if [ "${alist_watchdog}" == "1" ]; then
    ...
    rm -rf /tmp/alist.pid
    start-stop-daemon --start --quiet --make-pidfile --pidfile /tmp/alist.pid --background --startas /bin/sh -- -c "/koolshare/bin/alist --data ${AlistBaseDir} server >${ALIST_RUN_LOG} 2>&1"
    detect_running_status alist
  fi
}
